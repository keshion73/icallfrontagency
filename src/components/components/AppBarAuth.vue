      ...
      <v-row>
        ...
        <v-col cols="9" class="mx-auto text-center">
          <template v-if="!$vuetify.breakpoint.mobile">
            <v-menu v-for="item in links" :key="item.name" open-on-hover offset-y bottom min-width="360"
              max-width="600">
              <template v-slot:activator="{ on, attrs }">
                <v-btn v-bind="attrs" v-on="on" :ripple="false" color="transparent"
                  :class="{ 'btn-dark-hover': !hasBg, 'btn-hover': hasBg }" class="
                    text-white
                    font-weight-light
                    text-capitalize
                    ls-0
                    align-center
                  " depressed link :to="item.link">
                  {{ item.name }}
                  <v-img src="@/assets/img/down-arrow-white.svg" alt="down-arrow"
        ...
        <v-col cols="3" class="text-right">
          ...
          <v-btn v-show="$vuetify.breakpoint.mobile" elevation="0" :ripple="false" height="43" class="
              font-weight-600
              text-capitalize
              drawer-toggler
              btn-toggler-hover
              ...
              px-6
              rounded-sm
            " color="transparent" @click="dialog = true">
          ...
          <v-dialog v-model="dialog" content-class="position-absolute top-0" width="95%">
            <v-card class="card-shadow card-padding">
              ...
              <v-card-text class="card-padding pb-0 d-flex flex-column px-0">
                <v-btn v-for="item in links" :key="item.name" :ripple="false" text
                  class="text-typo text-capitalize ls-0" depressed link :to="item.link">
                  {{ item.name }}
                  <v-spacer></v-spacer>
                </v-btn>
