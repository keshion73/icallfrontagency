<template>
  <v-navigation-drawer width="100%" height="calc(100% - 2rem)" fixed app floating :expand-on-hover="mini"
    :value="drawer" :right="$vuetify.rtl" class="my-4 ms-4 border-radius-xl"
      ...
      <v-list-item-content class="pa-0">
        <v-list-item-title class="title d-flex align-center mb-0">
          <div class="v-navigation-drawer-brand pa-5 d-flex align-center">
            <v-img src="@/assets/img/logo-ct-white.png" class="navbar-brand-img ms-3" width="32"
              v-if="sidebarTheme == 'dark'">
            </v-img>
            <v-img src="@/assets/img/logo-ct.png" class="navbar-brand-img ms-3" width="32" v-else>
            </v-img>
            <span class="ms-2 font-weight-bold text-sm">iCall ADMIN</span>
          </div>
        </v-list-item-title>
      </v-list-item-content>
    </v-list-item>
    ...
    <v-list nav dense>
      <v-list-group :ripple="false" append-icon="fas fa-angle-down" class="pb-1 mx-2" active-class="item-active">
        ...
        <v-list-item :ripple="false" link class="mb-1 no-default-hover py-2" v-for="child in userInfo"
          :key="child.title">
    ...
    <v-list nav dense>
      <!-- <v-list-group :ripple="false" v-for="item in items" :key="item.title" v-model="item.active"
        append-icon="fas fa-angle-down" class="pb-1 mx-2" active-class="item-active">
        <template v-slot:activator>
        ...
        <v-list-item :ripple="false" link class="mb-1 no-default-hover px-0" :class="child.items ? 'has-children' : ''"
          v-for="child in item.items" :key="child.title" :to="child.link">
          <div class="w-100 d-flex align-center pa-2 border-radius-lg">
            ...
            <v-list-item-content class="ms-6 ps-1 py-0" v-if="child.items">
              <v-list-group prepend-icon="" :ripple="false" sub-group no-action v-model="child.active">
                <template v-slot:activator>
                  <v-list nav dense class="pa-0">
                    <v-list-group :ripple="false" append-icon="fas fa-angle-down me-auto ms-1"
                      active-class="item-active" class="mb-0">
                      <template v-slot:activator class="mb-0">
                        <v-list-item-content class="py-0">
                          <v-list-item-title v-text="child.title"></v-list-item-title>
                        </v-list-item-content>
      ...
      <!-- <v-list-group :ripple="false" v-for="item in menu_1depth" :key="item.menu_1level" v-model="item.active"
        append-icon="fas fa-angle-down" class="pb-1 mx-2" active-class="item-active">
        <template v-slot:activator>
          <v-list-item-icon class="me-2 align-center">
          ...
          <v-list-item-content>
            <v-list-item-title v-text="item.menu_name" class="ms-1"></v-list-item-title>
          </v-list-item-content>
        </template>

        <v-list-item :ripple="false" link class="mb-1 no-default-hover px-0" v-for="child in menu_2depth"
          :key="child.menu_2level" :to="child.menu_url">
          <v-list-item-content class="ps-4" v-if="item.menu_name == child.menu_1level">
            <div class="d-flex align-items-center pa-2">
              <v-list-item-title class="ms-6" v-text="child.menu_name" @click="listClose($event)"></v-list-item-title>
            </div>
          </v-list-item-content>
        </v-list-item>
      </v-list-group> -->

      <v-list-group :ripple="false" v-for="item in leftMenu" :key="item.menu_1level" v-model="item.active"
        append-icon="fas fa-angle-down" class="pb-1 mx-2" active-class="item-active">
        <template v-slot:activator>
          <v-list-item-content>
            <v-list-item-title v-text="item.menu_name" class="ms-1"></v-list-item-title>
        ...
        <v-list-item :ripple="false" link class="mb-1 no-default-hover px-0" v-for="child in item.child"
          :key="child.menu_2level" :to="child.menu_url">
      ...
      <!-- <v-list-group :ripple="false" v-for="item in itemsPages" :key="item.title" v-model="item.active"
        append-icon="fas fa-angle-down" class="pb-1 mx-2" active-class="item-active">
        <template v-slot:activator>
          <v-list-item-icon class="me-2 align-center">
            <i class="material-icons-round opacity-10">{{ item.action }}</i>
        ...
        <v-list-item :ripple="false" link class="mb-1 no-default-hover px-0" :class="child.items ? 'has-children' : ''"
          ...
          <v-list-item-content class="py-0" v-if="child.items">
            <v-list-group prepend-icon="" :ripple="false" sub-group no-action v-model="child.active">
              ...
              <v-list-item v-for="child2 in child.items" :ripple="false" :key="child2.title" :to="child2.link"
                @click="listClose($event)" class="px-0">
                <v-list-item-content>
                  <div class="d-flex align-items-center pa-2">
                    <span class="v-list-item-mini" v-text="child2.prefix"></span>
      ...
      <!-- <v-list-item-group>
        <div v-for="(item, i) in itemsDocs" :key="i">
          <v-list-item link :to="item.link" class="pb-1 mx-2 no-default-hover py-2" :ripple="false"
            active-class="item-active" v-if="!item.external">
            <v-list-item-icon class="me-2 align-center">
              <i class="material-icons-round opacity-10">{{ item.action }}</i>
            </v-list-item-icon>
            <v-list-item-content>
          ...
          <v-list-item link :href="item.link" class="pb-1 mx-2 no-default-hover py-2" :ripple="false"
            active-class="item-active" v-if="item.external" target="_blank">
            <v-list-item-icon class="me-2 align-center">
              <i class="material-icons-round opacity-10">{{ item.action }}</i>
            </v-list-item-icon>
            <v-list-item-content>
    ...
    <v-card class="pa-0 border-radius-lg mt-7 mb-9 mx-4"
...
export default {
  ...
  methods: {
    ...
    setMenuList() {
      this.menuList = this.GET_SESSION_INFO().menuList;
      // for (var i in this.menuList) {
      //   if (this.menuList[i].menu_2level == null) {
      //     this.menu_1depth.push(this.menuList[i]);
      //   }
      // }
      // if (this.menu_1depth.length > 0) {
      //   for (var i in this.menu_1depth) {
      //     for (var j in this.menuList) {
      //       if (this.menuList[j].menu_2level != null
      //         && this.menu_1depth[i].menu_name == this.menuList[j].menu_1level) {
      //         this.menu_2depth.push(this.menuList[j]);
      //       }
      //     }
      ...
      for (var i in this.menuList) {
        if (this.menuList[i].menu_2level == null) {
          this.leftMenu.push(this.menuList[i]);
        }
      }
      if (this.leftMenu.length > 0) {
        for (var i in this.leftMenu) {
          if (this.leftMenu[i].child == null) { this.leftMenu[i].child = []; }
          for (var j in this.menuList) {
            if (this.menuList[j].menu_2level != null
              && this.leftMenu[i].menu_name == this.menuList[j].menu_1level) {

              this.leftMenu[i].child.push(this.menuList[j])
            }
          }
        }
