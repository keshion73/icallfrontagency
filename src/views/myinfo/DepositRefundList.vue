        ...
        <v-card class="card-shadow border-radius-xl mt-1 mb-6" id="basic">
            <div class="px-6 pb-6 pt-0">
                <v-row class="mt-0">
                    ...
                    <v-col cols="2">
                        <v-row class="mt-0">
                            <label class="text-sm font-weight-bold mt-5">환불요청번호 : </label>
                            <v-col cols="8">
                                <v-text-field ref="refRefundKey" v-model="searchData.refund_key" color="#e91e63"
                                    placeholder="환불요청번호" class="font-size-input input-style mt-0 pt-0" outlined
        ...
        <v-card class="card-shadow">
            ...
            <v-card-text class="px-0 py-0">
                <v-data-table :headers="refundRequestHeaders" :items="depositRefundRequestList"
                    class="table charge_history_table" :page.sync="page" hide-default-footer
                    @page-count="pageCount = $event" :items-per-page="itemsPerPage" mobile-breakpoint="0" single-select>
                    <template #[`item.progress_name`]="{ item }">
                        <!-- <v-chip :color="getColor(item.progress_name)" dark @click="rowClick(item)">{{ item.progress_name
                                                                                                                        }}</v-chip> -->
